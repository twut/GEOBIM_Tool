...
def plotPoints(dataset):
    plt.plot(dataset[:,0],dataset[:,1],'o',markersize=2,markerfacecolor='0.75',
            markeredgewidth=2)
    plt.axis('equal')
...
def plotPath(dataset, path):
    plt.plot(dataset[:,0],dataset[:,1],'o',markersize=5,markerfacecolor='0.65',
            markeredgewidth=0)
    path = np.asarray(path)
    plt.plot(path[:,0],path[:,1],'o',markersize=5,markerfacecolor='0.55',
            markeredgewidth=0)
...
def concaveHull(dataset, k, if_optimal=False):
    ...
    while ( (not np.array_equal(firstpoint, currentPoint) or (step==2)) and points.size > 0 ):
        ...
        while ( (its==True) and (i<cPoints.shape[0]) ):
                i=i+1
                if ( np.array_equal(cPoints[i-1], firstpoint) ): # if next concave pt is the first pt
                    lastPoint = 1
                else:
                    lastPoint = 0
                j = 2
...
def PlotHull(hull):
    ...
    plt.suptitle("ConcaveTest")
    plt.show()
def PlotHullAndPoints(hull,dataset):
    ...
    for i in range(len(hull)-1):
        ...
        y2 = hull[i+1].item(1)
        plt.plot([x, x2], [y, y2], color='r')
    plt.suptitle("Concave Hull and points")
    plt.show()
